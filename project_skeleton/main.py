...
def count_neighboring_bomb(row_num, col_num):
    ...
    if col_num>0 and cells[row_num][col_num-1].bomb==True:
        cells[row_num][col_num].neighbouring_bombs +=1
    if col_num>0 and row_num<amount_of_cells-1 and cells[row_num+1][col_num-1].bomb ==True:
        cells[row_num][col_num].neighbouring_bombs +=1         
    if col_num<amount_of_cells-1 and row_num>0 and cells[row_num-1][col_num+1].bomb==True :
        cells[row_num][col_num].neighbouring_bombs +=1
    if col_num<amount_of_cells-1 and cells[row_num][col_num+1].bomb==True :
        cells[row_num][col_num].neighbouring_bombs +=1
    if row_num<amount_of_cells-1 and col_num<amount_of_cells-1 and cells[row_num+1][col_num+1].bomb==True:
        cells[row_num][col_num].neighbouring_bombs +=1

def draw_neighboring_bomb_counts(row_num, col_num):
    neiboring_bomb_counter_text = COUNT_NEIGHBORING_BOMB_FONT.render(str(cells[row_num][col_num].neighbouring_bombs),1, WHITE)
    screen.blit(neiboring_bomb_counter_text,
                (cells[row_num][col_num].cell_center[0]-(neiboring_bomb_counter_text.get_width()//2),
                 cells[row_num][col_num].cell_center[1]-(neiboring_bomb_counter_text.get_height()//2)))

def draw_score():
    score_text = COUNT_NEIGHBORING_BOMB_FONT.render(f"SCORE: {selected_cells_count}",1, WHITE)
...
def draw_gameend_text(text):
    ...
    screen.blit(gameover_text, ((READJUSTED_SIZE-gameover_text.get_width())/2, (READJUSTED_SIZE-gameover_text.get_height())//2))
    GAMEEND = True
    restart_text = GAMEEND_FONT2.render("** Press key A to see the answer! **",1,RED)
    screen.blit(restart_text, ((READJUSTED_SIZE-gameover_text.get_width())/2, (READJUSTED_SIZE-gameover_text.get_height())//2+40))
    restart_text = GAMEEND_FONT2.render("** Press key S to Restart! **",1,RED)
    screen.blit(restart_text, ((READJUSTED_SIZE-gameover_text.get_width())/2, (READJUSTED_SIZE-gameover_text.get_height())//2+80))
